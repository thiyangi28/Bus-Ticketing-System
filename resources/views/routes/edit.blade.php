...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-3xl shadow-xl shadow-slate-200/50 p-8 border border-slate-100">
        <h2 class="text-2xl font-bold text-slate-900 mb-6">Edit Route Details</h2>
        ...
        <form action="{{ route('routes.update', $route->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="route_number" required placeholder="e.g. EX 1-1" value="{{ old('route_number', $route->route_number) }}"
            ...
            <div class="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="start_point" required placeholder="e.g. Colombo" value="{{ old('start_point', $route->start_point) }}"
                        class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-4 focus:ring-indigo-100 focus:border-indigo-500 transition-all outline-none">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">End Point</label>
                    <input type="text" name="end_point" required placeholder="e.g. Kandy" value="{{ old('end_point', $route->end_point) }}"
                        class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-4 focus:ring-indigo-100 focus:border-indigo-500 transition-all outline-none">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="distance" placeholder="e.g. 115" value="{{ old('distance', $route->distance) }}"
                        class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-4 focus:ring-indigo-100 focus:border-indigo-500 transition-all outline-none">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Estimated Duration</label>
                    <input type="text" name="duration" placeholder="e.g. 3 hours 30 mins" value="{{ old('duration', $route->duration) }}"
                        class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-4 focus:ring-indigo-100 focus:border-indigo-500 transition-all outline-none">
                </div>
            </div>
